...
def train_test_generator(train, test, NTEST=7, NTRAIN=1, additional=False, verbose=False):
    for test_iteration in range(NTEST):
        test_sample = sample_test(train, test, seed=test_iteration, verbose=verbose)
        print(f"Test sample {test_iteration}")
        for train_iteration in range(NTRAIN):
            print(f"Train sample {train_iteration}")
            train_sample = sample_train(train, test, test_sample, include_other_peptides=additional, verbose=verbose)
            yield dict(
