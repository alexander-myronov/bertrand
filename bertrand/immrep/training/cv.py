...
def train_test_generator(train, test, NTEST=7, NTRAIN=1, additional=False):
    for test_iteration in range(NTEST):
        ...
        for train_iteration in range(NTRAIN):
            ...
            train_sample = sample_train(train, test, test_sample, include_other_peptides=additional)
            yield dict(
